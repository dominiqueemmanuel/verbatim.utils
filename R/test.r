# verbatim_rv<-list()
# verbatim_rv$global_table<-data.frame(`Libellé thème`=paste("Cluster ",seq(20)),Occurences=100,stringsAsFactors = FALSE,check.names = FALSE)
# verbatim_rv$object<-object
# pdf("test.pdf",width=20,height=ceiling(20/(2)))
# for(kk in 1:20){
#   cat(".")
# tab1<-verbatim_rv$object$rule_table%>%subset(topic==kk)
# z<-tab1$terms
#
...
#              ,top =textGrob(paste0(verbatim_rv$global_table$`Libellé thème`[kk]," \n(",verbatim_rv$global_table$Occurences[kk]," occurences)"),gp=gpar(fontsize=20,font=3))
...
#              ,x$p_tree
#              , ncol=2,top =textGrob(paste0(verbatim_rv$global_table$`Libellé thème`[kk]," \n(",verbatim_rv$global_table$Occurences[kk]," occurences)"),gp=gpar(fontsize=20,font=3))
#              )
# vps <- baseViewports()
